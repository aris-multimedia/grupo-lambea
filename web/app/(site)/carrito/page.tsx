...
import { subtotal, totalDiscount, total, freeUnitsFor, unitsForFamilia } from '@/lib/cart';
...
export default function CarritoPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-6 text-center px-6">
        ...
        <div>
          <h1 className="text-[24px] font-semibold text-[var(--ink)] mb-2">Tu cesta está vacía</h1>
          <p className="text-[15px] text-[var(--ink-500)]">Explora nuestra tienda y añade productos</p>
        </div>
        <Link
          href="/tienda"
  ...
  const slugsWithPromo = [...new Set(items.map(i => i.slug))].filter(
    slug => freeUnitsFor(items, slug) > 0
  );
  ...
  return (
    <div className="max-w-[1320px] mx-auto px-4 md:px-8 py-8 md:py-12">
      <h1 className="text-[22px] md:text-[28px] font-semibold text-[var(--ink)] mb-6 md:mb-8">
        Tu cesta <span className="text-[var(--ink-500)] font-normal text-[16px] md:text-[18px]">({items.reduce((s, i) => s + i.cantidad, 0)} artículos)</span>
      </h1>
      ...
      <div className="grid gap-6 md:gap-8 grid-cols-1 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-3">
          {items.map((item) => {
            const totalUnits = unitsForFamilia(items, item.slug);
            const freeCount = freeUnitsFor(items, item.slug);
            const hasPromo = freeCount > 0;
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {hasPromo && (
                      <div className="inline-flex items-center gap-1.5 mt-1.5 text-[11px] font-semibold text-[#92400e] bg-[#fffbeb] px-2 py-0.5 rounded-full">
                        <Tag size={10} />
                        3×2 — {freeCount} {freeCount === 1 ? 'unidad gratis' : 'unidades gratis'}
                      </div>
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: '1px solid var(--line)' }}>
                  ...
                  >
                    <button
                      ...
                      className="w-9 h-9 flex items-center justify-center text-[var(--ink-700)] hover:bg-[var(--blue-soft)] hover:text-[var(--blue)] transition-colors"
                  ...
                  <div className="text-right">
                    ...
                    {item.cantidad > 1 && (
                      <div className="text-[11px] text-[var(--ink-500)]">
                        {item.precio.toFixed(2).replace('.', ',')} € / ud
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="space-y-4 sticky top-[100px] self-start">
          <div className="bg-white rounded-xl border border-[var(--line)] p-6">
            <h2 className="text-[16px] font-semibold text-[var(--ink)] mb-5">Resumen</h2>
          ...
          {slugsWithPromo.length > 0 && (
            ...
            >
              ...
              <div className="text-[var(--ink-500)]">
                Estás ahorrando <strong className="text-[#92400e]">{disc.toFixed(2).replace('.', ',')} €</strong>
              </div>
            </div>
          )}
        </div>
