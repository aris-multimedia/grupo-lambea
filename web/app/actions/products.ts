...
export async function updateProduct(slug: string, formData: FormData) {
  ...
  const precio_desde = formData.get('precio_desde') ? Number(formData.get('precio_desde')) : null;
  const precio_hasta = formData.get('precio_hasta') ? Number(formData.get('precio_hasta')) : null;
  ...
  for (const v of variants) {
    if (v.id != null) {
      ...
      await sql`
        ...
        VALUES (${productId}, ${v.formato}, ${v.precio}, ${v.imagen_url ?? null}, ${v.orden})
      `;
    }
  }
