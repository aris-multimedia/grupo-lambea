...
export function ProductCard({ product, priority = false }: Props) {
  ...
  function handleAdd(e: React.MouseEvent) {
    ...
    addItem({
      ...
      aplicacion: product.aplicaciones[0] ?? 'generico',
      formato: product.formatos?.[0] ?? 'Unidad',
      precio: product.precio_desde ?? 0,
      imagen: product.imagen ?? '/assets/productos/prod-nautico-01.jpg',
    })
  }
    ...
    >
      ...
      <div className="relative aspect-square bg-white overflow-hidden flex items-center justify-center">
        ...
        {promo.activa && promo.tipo === 'descuento' && pct > 0 && (
          ...
          >
            <span className="font-(family-name:--font-lora) font-bold text-[var(--ink)] leading-none text-[13px] sm:text-[16px]">−{pct}%</span>
            <span className="text-[7px] sm:text-[8px] font-bold uppercase tracking-[0.04em] text-[var(--ink)] opacity-75 mt-0.5">Dto.</span>
          </div>
        )}
      </div>
      ...
      <div className="px-3.5 sm:px-[22px] pt-3.5 sm:pt-[22px] pb-4 sm:pb-6 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between pt-2.5 sm:pt-3.5 border-t border-[var(--line)] gap-2">
          {product.precio_desde ? (
            ...
            <span className="text-[12px] sm:text-[13px] text-(--ink-500) italic">Consultar</span>
          )}
          <button
            type="button"
            onClick={handleAdd}
