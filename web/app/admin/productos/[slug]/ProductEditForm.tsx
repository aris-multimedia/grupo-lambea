...
const inputClass =
  'w-full border border-[#d1d5db] rounded-lg px-3.5 py-2.5 text-[14px] outline-none focus:border-[#1E92D8] focus:ring-2 focus:ring-[#1E92D8]/10 transition-all bg-white';
const labelClass = 'block text-[12px] font-semibold uppercase tracking-wider text-[#6b7280] mb-2';
  ...
  return (
    <div className="min-h-screen bg-[#f4f6f9]">
      <form ref={formRef} onSubmit={handleSubmit}>
        {/* ── STICKY HEADER ─────────────────────────────────── */}
        <div className="sticky top-0 z-20 bg-white border-b border-[#e5e7eb] px-8 py-4 flex items-center justify-between shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="submit"
              disabled={pending}
              className="inline-flex items-center gap-2 bg-[#1a1f2a] hover:bg-[#2d3748] disabled:opacity-60 text-white font-semibold text-[13px] px-5 py-2 rounded-lg transition-colors"
            >
              <Save size={14} />
        ...
        <div className="max-w-[1200px] mx-auto px-8 py-8 grid gap-6 grid-cols-1 xl:grid-cols-[1fr_280px]">
          {/* ── LEFT ──────────────────────────────────────────── */}
          ...
          {/* ── RIGHT: SIDEBAR ──────────────────────────────── */}
          <div className="space-y-5 xl:sticky xl:top-[73px] self-start">
            ...
            <Section title="Precios (IVA inc.)">
              <div className="space-y-3">
                <Field label="Desde (€)">
                  <input name="precio_desde" type="number" step="0.01" min="0" defaultValue={product.precio_desde ?? ''} className={inputClass} placeholder="Auto" />
                </Field>
                <Field label="Hasta (€)">
                  <input name="precio_hasta" type="number" step="0.01" min="0" defaultValue={product.precio_hasta ?? ''} className={inputClass} placeholder="Auto" />
                </Field>
  ...
  return (
    <section className="bg-white rounded-xl border border-[#e5e7eb] overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[#f3f4f6] flex items-center gap-2">
        ...
        <h2 className="text-[13px] font-semibold text-[#1a1f2a]">{title}</h2>
      </div>
...
function Toggle({
  ...
  return (
    <button
      ...
      style={checked ? { borderColor: '#1E92D8', background: 'rgba(30,146,216,0.06)' } : { borderColor: '#e5e7eb', background: '#fff' }}
    >
      <div
        className="w-9 h-5 rounded-full relative transition-all shrink-0"
        style={{ background: checked ? '#1E92D8' : '#d1d5db' }}
      >
        <div
      ...
      <div>
        <div className="text-[13px] font-medium text-[#1a1f2a]">{label}</div>
      ...
      <div className="grid grid-cols-[20px_16px_1fr_110px_32px] gap-3 items-center px-4 pt-2.5 pb-1.5">
        ...
        <div className="relative">
          <input
            ...
            onChange={(e) => onChange(idx, 'precio', e.target.value ? Number(e.target.value) : null)}
